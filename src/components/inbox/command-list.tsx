...
export default function CommandList({ initial }: { initial: UnifiedList }) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (!inFlight.current && !editingId && !schedId) pull();
    }, 5000);
    return () => clearInterval(t);
  }, [pull, editingId, schedId]);
            ...
            <div key={`${item.source}-${item.id}`} className="card p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.source === "todo" && item.when && !item.done ? <div className="text-xs text-ink-faint mt-0.5">due {fmtWhen(item.when)}</div> : null}
